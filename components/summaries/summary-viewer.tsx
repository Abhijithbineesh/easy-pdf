...
export function SummaryViewer({ summary }: { summary: string }) {
  ...
  // ✅ Make sure intro without "#" is treated as its own section
  const rawSections = summary.includes("\n#")
    ? summary.split("\n#")
    : [summary];

  const sections = rawSections.map((section, index) => {
    const normalized =
      index === 0 && !section.trim().startsWith("#")
        ? `# Quick Overview\n${section}`
        : `#${section}`;

    return parseSection(normalized.trim());
  });
